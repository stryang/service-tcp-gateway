 ...
 *
 * @author yq
 * @date 2021-04-14 13:12
 */
@Slf4j
@Configuration
@EnableWebSocket
public class StgWebSocketConfig implements WebSocketConfigurer {

    @Autowired
    private StgWebsocketHandshakeConfig stgWebsocketHandshakeConfig;

    @Autowired
    private StgWebSocketHandler stgWebSocketHandler;
    ...
    public void registerWebSocketHandlers(WebSocketHandlerRegistry registry) {
         ...
         */
        registry.addHandler(stgWebSocketHandler, "/echo")
                .addInterceptors(stgWebsocketHandshakeConfig)
                .setAllowedOrigins("*");

        /**
         * 部分 不支持websocket的访问链接,允许跨域
         */
        registry.addHandler(stgWebSocketHandler, "/sockjs/echo")
                .addInterceptors(stgWebsocketHandshakeConfig)
                .setAllowedOrigins("*")
                .withSockJS();
    }
